...
class JupyterServer {
    ...
    public start(): Promise<JupyterServer.IInfo> {
        ...
        this._startServer = new Promise<JupyterServer.IInfo>((resolve, reject) => {
            ...
            this._nbServer = execFile(this._info.environment.path, ['-m', 'jupyter', 'notebook', '--no-browser'],
              { cwd: process.env.KISIA_JUPYTERLAB_CWD,
                env: env
              }
            );
            ...
            this._nbServer.on('exit', () => {
                ...
                log.warn("Failed to start server, received exit")
            ...
            this._nbServer.stdout.on('data', (serverBuff: string) => {
              log.info("STDOUT",serverBuff.toString());
            })

            this._nbServer.stderr.on('data', (serverBuff: string) => {
              log.info("STDERR",serverBuff.toString());
              log.info("Server received info");
                let urlMatch = serverBuff.toString().match(urlRegExp);
                if (!urlMatch) {
                  log.info("no url match against",urlRegExp);
                ...
                log.info("Resolving - server has started" , this._info.url)
                resolve(this._info);
            });
        });
...
class JupyterServerFactory implements IServerFactory, IClosingService {
    ...
    private _createServer(opts: JupyterServer.IOptions): JupyterServerFactory.IFactoryItem {
      log.info("createServer options",opts)
        let item: JupyterServerFactory.IFactoryItem = {
