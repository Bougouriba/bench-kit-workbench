...
run_batch_packages_job() (
	...
	local MAX=$KXX_MAX_PARALLEL
	if [ $MAX = 1 ]; then
		for x in $ELT_LIST; do
			echo "$SCOPE:$x -> $CMD"
			cd $BASE/$x && $CMD > $KXX_LOGS/$SCOPE-$x.log 2>&1
		done
	else
		local pids=[]
		local COUNT=0
		...
		for x in $ELT_LIST; do
			...
			echo "   $x -> $CMD , logs $KXX_LOGS/$LOGFILENAME"
			#cd $BASE/$x && $CMD > $KXX_LOGS/$LOGFILENAME 2>&1 &
			run_one "$BASE/$x" "$KXX_LOGS/$LOGFILENAME" $CMD &
			pids[${i}]=$!
			COUNT=$(expr $COUNT + 1 )
